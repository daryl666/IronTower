                ...
                <ul class="breadcrumb" style="margin-bottom: 0;background-color: #F5F5F5">
                    ...
                    <li>
                        @if(Auth::user()->area_level == '湖北省')                             <a
                                href="{{url('backend/siteShield/checkShieldPage?region=').Auth::user()->area_level.'&checkStatus=1&reqType=0&beginDate=&endDate='}}">屏蔽记录管理</a>@endif                         @if(Auth::user()->area_level != '湖北省')
                            <a href="{{url('backend/siteShield/addShieldPage')}}">屏蔽记录管理</a>@endif
                    </li>
                    <li class="active">
                        <a href="#">解屏蔽记录查询</a>
    ...
    <div class="list">
        <div class="body">
            <form id="listForm" method="post" action="{{url('backend/siteShield')}}" enctype="multipart/form-data">
                ...
                <div class="listBar">
                    <label style="margin-bottom:10px">选择起止月份和地市来查看解屏蔽记录：</label><br>
                    ...
                    {{--<td>--}}
                    {{--&nbsp;&nbsp;&nbsp;&nbsp;记录类型：--}}
                    {{--<input type="radio" name="reqType" value="0"/>屏蔽记录--}}
                    {{--<input type="radio" name="reqType" value="1" checked="checked"/>解屏蔽记录--}}
                    {{--</td>--}}
                    <td>
                        &nbsp;&nbsp;&nbsp;&nbsp;
        ...
        <div id="siteCheck">
            <table class="listTable" style="white-space:nowrap;font-size:12px;">
                ...
                <tr>
                    ...
                    <th>
                        <a href="#" class="sort" name="" hidefocus>屏蔽原因</a>
                    </th>
                    <th>
                        <a href="#" class="sort" name="" hidefocus>屏蔽开始时间</a>
                    </th>
                    <th>
                        <a href="#" class="sort" name="" hidefocus>解屏蔽时间</a>
                    </th>
                    <th>
                    ...
                    <th>
                        <a href="#" class="sort" name="" hidefocus>拆迁结束时间</a>
                    </th>
                    <th>
                        <a href="#" class="sort" name="" hidefocus>拆迁原因</a>
                    </th>
                    <th>
                        <a href="#" class="sort" name="" hidefocus>拆迁之后的基站号</a>
                    </th>
                    <th>
                        <a href="#" class="sort" name="" hidefocus>拆迁之后的基站名称</a>
                    </th>
                    <th>
                        <a href="#" class="sort" name="" hidefocus>拆迁之后的站址编码</a>
                    </th>
                ...
                @if(isset($siteShields))
                    @foreach($siteShields as $siteShield)
                        <tr>
                            @if(Auth::user()->area_level != '湖北省')
                                <td>
                                    <button class="buttonNextStep" onclick="doWithdraw({{$siteShield->id}})">撤回申请
                                    </button>
                                    {{--@if(Auth::user()->area_level == '湖北省')--}}
                                    {{--<a href="javascript:void(0)" onclick="doApprove({{$siteShield->id}})">通过</a>&nbsp;--}}
                                    {{--&nbsp;&nbsp;--}}
                                    {{--<a href="javascript:void(0)" onclick="doDeny({{$siteShield->id}})">驳回</a>@endif--}}
                                </td>@endif
                            ...
                            <td>{{$siteShield->shield_end_time}}</td>
                            <td>@if($siteShield->shield_reason == transShieldReason('故障')){{$siteShield->shield_start_time}}@endif</td>
                            <td>@if($siteShield->shield_reason == transShieldReason('故障')){{$siteShield->shield_end_time}}@endif</td>
                            <td>{{transOsReason($siteShield->os_reason)}}</td>
                            <td>{{$siteShield->os_detail}}</td>
                            <td>{{transRespUnit($siteShield->response_unit)}}</td>
                            <td>{{$siteShield->demolition_start_time}}</td>
                            <td>@if($siteShield->shield_reason != transShieldReason('故障')){{$siteShield->shield_end_time}}@endif</td>
                            <td>{{transDemReason($siteShield->demolition_reason)}}</td>
                            <td>{{$siteShield->new_station_code}}</td>
                            <td>{{$siteShield->new_station_name}}</td>
                            <td>{{$siteShield->new_site_code}}</td>
                        </tr>
                    @endforeach
...
@section('script_footer')
    <script type="text/javascript">
        ...
        function doSearch() {
            ...
            var url = "{{url('backend/siteShield/unshieldPage?region=')}}" + region + '&beginDate=' + beginDate + '&endDate=' + endDate + '&checkStatus=' + checkStatus;
            listForm.method = "GET";
            listForm.action = url;
            listForm.submit();
        }
        ...
        function doWithdraw(id) {
            var listForm = document.getElementById("listForm");
            var url = "{{url('backend/siteShield/withdrawUnshield')}}" + '/' + id;
            listForm.action = url;
            listForm.submit();
        }
        ...
        function doApprove(id) {
            var listForm = document.getElementById("listForm");
            var url = "{{url('backend/siteShield/approveUnshield')}}" + '/' + id;
            listForm.action = url;
            listForm.submit();
        ...
        function doDeny(id) {
            var listForm = document.getElementById("listForm");
            var url = "{{url('backend/siteShield/denyUnshield')}}" + '/' + id;
            listForm.action = url;
            listForm.submit();
