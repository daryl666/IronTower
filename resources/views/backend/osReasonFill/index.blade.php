...
@section('content')
    ...
    {{--<input type="button" value="短时退服填报" onclick="doOsreasonFillPage()">--}}
    {{--</td>--}}
    {{--</div>--}}
        ...
        <form id="listForm" method="get" action="{{url('backend/osReasonFill?region=').Auth::user()->area_level.'&checkStatus=0&beginDate=&endDate='}}">
            <div class="body">
                ...
                <div class="listBar">
                    ...
                    <td>
                        &nbsp;&nbsp;&nbsp;&nbsp;处理状态：
                        <input type="radio" name="checkStatus" value="0" checked="checked"/>未处理
                        <input type="radio" name="checkStatus" value="1"/>已处理
                    </td>
                    <td>
                        &nbsp;&nbsp;&nbsp;&nbsp;
                ...
                <table class="listTable" style="white-space:nowrap;font-size:12px;">
                    ...
                    @if(isset($osReasons))
                        @foreach($osReasons as $osReason)
                            <tr>
                                <td>
                                    <button class="buttonNextStep" onclick="doAdd({{$osReason->id}})">提交</button>
                                </td>
                                ...
                                <td>
                                    <select name="osReason_{{$osReason->id}}">
                                        <option>停电</option>
                                        <option>电源设备</option>
                                        <option>传输线路</option>
                                        <option>传输设备</option>
                                ...
                                <td>
                                    <textarea name="osDetail_{{$osReason->id}}" style="height: 40px"
                                              wrap="physical"></textarea>
                                    {{--<input type="text" name="osDetail_{{$osReason->id}}">--}}
                                </td>
                                <td>
...
@section('script_footer')
    <script type="text/javascript">
        ...
        function doAdd(id) {
            if (confirm('确认提交吗？')) {
                var listForm = document.getElementById("listForm");
                var url = "{{url('backend/osReasonFill/add')}}" + '/' + id;
                listForm.action = url;
                listForm.submit();
            }
            return;
